...
> Implements `docs/framework/03_EVIDENCE_STANDARDS.md (§3.4)` / audit standard §2.4. **No guess entries.** Each record carries evidence, evidence class, confidence, verification status, last-reviewed.
...
**Register rules (see `docs/framework/03_EVIDENCE_STANDARDS.md`):** no entry above 0.8 Confidence may remain PENDING at certification time; new assumptions surfaced in later phases are appended here before use.