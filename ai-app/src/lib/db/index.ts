...
async function ensureSchema(env: { DB: D1Database }): Promise<void> {
  ...
  // Also require api_cost_logs so the cost-log table gets created on DBs where
  // `workers` was fully migrated before this table existed.
  try {
    ...
    if (SCHEMA_COLS.every(c => names.includes(c)) && tables.includes("api_cost_logs")) {
      g[DONE_FLAG] = true;
      return;
    }
  } catch {}
  ...
  try {
    ...
    if (!g[DONE_FLAG] && !g[PENDING_FLAG]) {
      ...
      try {
        await Promise.race([
          ensureSchema({ DB: db }),
          new Promise((_, reject) => setTimeout(() => reject(new Error("Schema init timeout")), 90000)),
        ]);
        g[DONE_FLAG] = true;
      } catch (e) {
