...
export async function getPersonalizedInsights(workerId: string): Promise<PersonalizedInsight[]> {
  try {
    ...
    try {
      ...
      totalCourses = orderedCourseIds.results.length;
    } catch {
      totalCourses = 0;
    }
    ...
    if (sortedCats.length > 0) {
      ...
      if (topScore > 50) {
        insights.push({
          ...
          title: totalPurchases === 0 ? `Start learning ${topCat.replace(/_/g, " ")}` : `Deepen your ${topCat.replace(/_/g, " ")} skills`,
          titleBn: totalPurchases === 0 ? `${topCat.replace(/_/g, " ")} শেখা শুরু করুন` : `আপনার ${topCat.replace(/_/g, " ")} দক্ষতা আরও বাড়ান`,
          priority: totalPurchases === 0 ? 1 : 2,
          actionUrl: `/courses?interest=${topCat}`,
          reason: totalPurchases === 0 ? `Strong interest (${topScore}) in ${topCat} with no purchases yet` : `Continuing interest (${topScore}) in ${topCat}`,
    ...
    try {
      ...
      // affiliate_tree table may not exist
    }
