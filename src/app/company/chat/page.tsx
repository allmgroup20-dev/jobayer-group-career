...
export default function CompanyChatPage() {
      ...
      .then((d: any) => {
        if (d.conversations) {
          ...
          setStats({
            ...
            today: d.conversations.filter((c: any) => new Date(c.last_time).toDateString() === today).length,
            active: d.conversations.filter((c: any) => c.msg_count > 2).length,
          });
        }
      })
      .catch(() => {})
  ...
  const sendReply = useCallback(async () => {
    ...
    await fetch("/api/chat/send", {
      ...
      body: JSON.stringify({ phone: selectedPhone, text, agentName: "admin" }),
    });
  }, [replyText, selectedPhone]);
    ...
    <div className="h-[calc(100vh-8rem)] flex bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="w-80 shrink-0 border-r border-gray-200 flex flex-col bg-gray-50">
        ...
        <div className="p-3 border-b border-gray-200 bg-white space-y-2">
          <h2 className="font-bold text-gray-800">Chat Inbox</h2>
          ...
          <input
            ...
            className="w-full bg-gray-100 rounded-lg px-3 py-1.5 text-xs outline-none focus:ring-2 focus:ring-primary/30"
          />
        </div>
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="p-4 text-center text-gray-500 text-sm mt-8">No conversations</div>
          ) : filtered.map((c) => (
            <button
              key={c.phone}
              onClick={() => setSelectedPhone(c.phone)}
              className={`w-full text-left px-3 py-2.5 border-b border-gray-100 hover:bg-white transition-colors cursor-pointer ${
                selectedPhone === c.phone ? "bg-white shadow-sm" : ""
              }`}
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-accent text-white flex items-center justify-center text-sm font-bold shrink-0">
                  {(c.name || c.phone)[0].toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-baseline">
                    <span className="font-semibold text-sm text-gray-800 truncate">{c.name || c.phone}</span>
                    <span className="text-[10px] text-gray-400 shrink-0">{timeAgo(c.last_time)}</span>
                  </div>
                  <div className="text-xs text-gray-500 truncate mt-0.5">{c.last_message?.slice(0, 60)}</div>
                </div>
              </div>
            </button>
          ))}
            ...
            <div className="px-4 py-3 border-b border-gray-200 bg-white flex items-center gap-3 shrink-0">
              ...
              <div>
                ...
                <div className="text-[11px] text-gray-400">
                  {messages.filter(m => m.role === "user").length} messages from user
                </div>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto px-4 py-3 bg-[#ECE5DD] space-y-2">
              {messages.length === 0 ? (
                <div className="text-center text-gray-400 text-sm mt-12">No messages yet</div>
              ) : messages.map((msg) => (
