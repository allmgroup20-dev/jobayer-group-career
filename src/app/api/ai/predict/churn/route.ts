...
async function predictAllWorkers(db: D1Database) {
  const workers = await db.prepare("SELECT worker_id, name, phone, membership_status FROM workers WHERE membership_status = 'active'").bind().all() as { results: WorkerData[] };
