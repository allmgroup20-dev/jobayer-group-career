...
export async function GET() {
  try {
    ...
    const [brainUsage, feedback, flows, memory, schedules] = await Promise.all([
      ...
      query<Record<string, any>>(db, `SELECT id, name, description, steps, department_ids, created_by, is_active, run_count, last_run_at, created_at, updated_at FROM custom_flows ORDER BY created_at DESC LIMIT 200`),
      ...
      query<Record<string, any>>(db, `SELECT id, phone, agent_id, task_type, cron_expression, params, enabled, last_run_at, next_run_at, created_at FROM agent_schedule ORDER BY created_at DESC LIMIT 200`),
    ]);
