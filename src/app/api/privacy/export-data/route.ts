...
export async function GET(req: NextRequest) {
  try {
    ...
    const [worker, events, sessions, searches, interests, behavior, orders, reviews, devices, comms, consents] = await Promise.all([
      ...
      db.prepare("SELECT id, order_id, worker_id, product_id, product_name, quantity, total_amount, currency, payment_method, payment_status, commission_status, order_status, shipping_address, transaction_id, created_at FROM orders WHERE worker_id = ? ORDER BY created_at DESC").bind(workerId).all() as Promise<any>,
      db.prepare("SELECT id, worker_id, product_id, product_type, rating, review_text, is_approved, created_at FROM product_reviews WHERE worker_id = ? ORDER BY created_at DESC").bind(workerId).all() as Promise<any>,
      db.prepare("SELECT id, worker_id, device_type, browser, os, user_agent, screen_resolution, ip_address, city, country, timezone, language, is_active, last_seen_at, first_seen_at, created_at FROM user_devices WHERE worker_id = ?").bind(workerId).all() as Promise<any>,
      db.prepare("SELECT id, worker_id, channel, direction, message, status, reference_id, metadata, sent_at, created_at FROM communication_history WHERE worker_id = ? ORDER BY created_at DESC LIMIT 500").bind(workerId).all() as Promise<any>,
